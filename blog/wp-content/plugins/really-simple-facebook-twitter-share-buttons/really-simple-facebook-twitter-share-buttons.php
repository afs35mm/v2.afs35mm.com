...
function really_simple_share ($content, $filter, $link='', $title='') {
	...
	if ($option['active_buttons']['facebook_like']==true) {
		...
		$out .= '<div style="width:'.$option['facebook_like_width'].'px;" class="really_simple_share_facebook_like"> 
				<iframe style="height:30px;" src="http://www.facebook.com/plugins/like.php?href='.urlencode($link).'&amp;layout='.$option_layout.'&amp;show_faces=false&amp;width='.$option['facebook_like_width'].'&amp;action='.$option_facebook_like_text.'&amp;colorscheme=light&amp;send=false&amp;height='.$option_height.'" 
		...
		if ($option['facebook_like_send']) {
			...
			$out .= '<div style="float:left; width:50px; height:30px; padding-left:10px;" class="really_simple_share_facebook_like_send">
